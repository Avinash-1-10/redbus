...
const SeatDetails = ({ yourbookedSeats }) => {
  return (
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="border p-3 bg-white rounded-md flex flex-col gap-2">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex gap-2 flex-wrap">
            {yourbookedSeats.map((seat) => (
              <span className="px-2 bg-blue-500 text-white rounded-md text-[14px]" key={seat}>
                {seat}
              </span>
            ))}
          </div>
        ...
        <div className="flex flex-col">
            <h2 className="text-16px font-bold text-gray-600 mb-1">Fare Details</h2>
            <div className="flex justify-between items-center">
                <div className="flex flex-col items-center">
                    <span className="font-semibold">Total amount</span>
                    <span className="text-[14px] text-gray-600">Inc. all taxes</span>
                </div>
                <span className="text-[17px] font-bold text-green-500">INR {1800}</span>
            </div>
        </div>
        <hr />
        <div className="flex justify-center items-center">
        <button className="bg-blue-500 w-fit px-3 py-2 rounded-md font-bold text-white">Proceed to book</button>
        </div>
      </div>
    </div>
  );
